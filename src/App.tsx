...
import { Upload, Download, Check, X, FileText, ChevronRight, Pencil, Layout, Zap, Shield, Clock, AlertCircle, Link } from 'lucide-react';
import './index.css';
...
export default function App() {
    ...
    const parseResume = useCallback((text: string): { resume: Resume; log: ParseLog[] } => {
        ...
        return {
            resume: { name, email, phone, linkedin, github, profile, skills, experience, education, projects, achievements },
            log
        };
    }, []);
    ...
    const sections: { key: keyof Resume; label: string; multi?: boolean }[] = [
                                        ...
                                        <div key={sec.key} className={`sec ${isOpen ? 'open' : ''} ${isEmpty ? 'empty' : 'filled'}`}>
                                            <button className="sec-head" onClick={() => setEditing(isOpen ? null : sec.key)}>
                                                <span className="sec-label">
                                                    {isEmpty ? <span className="dot red"></span> : <span className="dot green"></span>}
                                                    {sec.label}
                                            ...
                                            {isOpen && (
                                                <div className="sec-body">
                                                    {sec.key === 'skills' ? (
                                                        <input value={resume.skills.join(', ')} onChange={e => update('skills', e.target.value.split(',').map(s => s.trim()).filter(Boolean))} placeholder="React, Node.js, Python..." />
                                                    ) : sec.multi ? (
                                                        <textarea value={val} onChange={e => update(sec.key, e.target.value)} rows={4} placeholder={`Enter ${sec.label.toLowerCase()}...`} />
                                                    ) : (
                                                        <input value={val} onChange={e => update(sec.key, e.target.value)} placeholder={`Enter ${sec.label.toLowerCase()}...`} />
                                                    )}
                                                </div>
                                            )}
                                        </div>
